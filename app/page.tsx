...
function OddEvenView({month,cutoff,setMonth,setCutoff,rows,finalLeans,resultCheck}:{month:number;cutoff:number;setMonth:(value:number)=>void;setCutoff:(value:number)=>void;rows:{label:string;games:{value:string;lean:string}[]}[];finalLeans:{game:string;result:string}[];resultCheck:{day:number;matches:number;games:{game:string;predicted:string;actual:string;value:string;matched:boolean}[]}|null}) {
  return <section className="pattern-view"><div className="pattern-view-head"><div><p>Odd / even patterns</p><h3>Direct number classification</h3><span>Classifies each FB, GB, GL and DS value on the selected date.</span></div><PeriodControls month={month} cutoff={cutoff} setMonth={setMonth} setCutoff={setCutoff} dateMode="day"/></div><div className="special-rule"><strong>Classification rule</strong><span>01–09, 10, 11, 20, 22, 30, 33, 40, 44, 50, 55, 60, 66, 70, 77, 80, 88, 90, 99 and 100 are Double Lean. Every other value is classified directly as Odd Lean or Even Lean.</span></div><div className="upcoming-lean final-lean-panel"><header><div><strong>Final upcoming result</strong><span>Uses same-date history, prior 2026 months, the previous month, and earlier current-month results. The algorithm resolves every tie to one result.</span></div><small>{cutoff} {MONTHS[month-1]} 2026 · pattern-based estimate</small></header><div className="upcoming-lean-grid final-lean-grid">{finalLeans.map(item=><article key={item.game}><b>{item.game}</b><strong className={`final-lean-result final-${item.result.toLowerCase()}`}>{item.result}</strong></article>)}</div></div><div className="previous-check"><header><div><strong>Selected-date result check</strong><span>{resultCheck?`${resultCheck.day} ${MONTHS[month-1]} 2026 — same date as Final Upcoming Result`:`${cutoff} ${MONTHS[month-1]} 2026 has no actual results yet`}</span></div>{resultCheck&&<b>{resultCheck.matches}/4 matched</b>}</header>{resultCheck&&<div>{resultCheck.games.map(item=><article key={item.game} className={item.matched?'check-hit':'check-miss'}><strong>{item.game}</strong><span>Predicted {item.predicted}</span><span>Actual {item.value} · {item.actual}</span><b>{item.matched?'Match':'No match'}</b></article>)}</div>}</div><div className="parity-table lean-table"><table><thead><tr><th>Comparison date</th>{GAME_NAMES.map(name=><th key={name}>{name}</th>)}</tr></thead><tbody>{rows.map(row=><tr key={row.label}><td><b>{row.label}</b></td>{row.games.map((game,p)=><td key={GAME_NAMES[p]}><strong className="lean-value">{game.value||'—'}</strong><span className={`lean-badge ${game.lean==='Odd Lean'?'lean-odd':game.lean==='Even Lean'?'lean-even':game.lean==='Double Lean'?'lean-double':'lean-empty'}`}>{game.lean}</span></td>)}</tr>)}</tbody></table></div><footer>The result check compares the prediction and actual values for the same selected date; it does not guarantee a future draw.</footer></section>;
}
...
export default function Home() {
  ...
  const calculateFinalLeans=(targetMonth:number,targetDay:number)=>GAME_NAMES.map((game,p)=>{const labels=['Odd Lean','Even Lean','Double Lean'],scores=new Map(labels.map(label=>[label,0])),add=(value:string|undefined,weight:number)=>{if(!value)return;const label=classifyLean(value);scores.set(label,(scores.get(label)||0)+weight);},yearRecord=(file25?.records||[]).find(r=>r.month===targetMonth&&r.day===targetDay),monthRecord=(file26?.records||[]).find(r=>r.month===targetMonth-1&&r.day===targetDay);add(yearRecord?.games[p],8);add(monthRecord?.games[p],10);for(let m=1;m<targetMonth;m++){const record=(file26?.records||[]).find(r=>r.month===m&&r.day===targetDay);add(record?.games[p],2+4*m/Math.max(1,targetMonth-1));}const groups=[{records:(file25?.records||[]).filter(r=>r.month===targetMonth&&r.day<targetDay),weight:4},{records:(file26?.records||[]).filter(r=>r.month===targetMonth-1&&r.day<=targetDay),weight:6},{records:(file26?.records||[]).filter(r=>r.month===targetMonth&&r.day<targetDay),weight:7}];groups.forEach(group=>{const values=group.records.map(r=>r.games[p]).filter(Boolean);values.forEach(value=>add(value,group.weight/Math.max(1,values.length)));});const history=[...(file25?.records||[]),...(file26?.records||[]).filter(r=>r.month<targetMonth||(r.month===targetMonth&&r.day<targetDay))],frequency=new Map(labels.map(label=>[label,0]));history.forEach(record=>{const label=classifyLean(record.games[p]);frequency.set(label,(frequency.get(label)||0)+1);});const recent=classifyLean(monthRecord?.games[p]||yearRecord?.games[p]||'');const winner=[...labels].sort((a,b)=>(scores.get(b)||0)-(scores.get(a)||0)||(a===recent?-1:0)-(b===recent?-1:0)||(frequency.get(b)||0)-(frequency.get(a)||0)||labels.indexOf(a)-labels.indexOf(b))[0];return {game,result:winner.replace(' Lean','')};});
  const finalLeans=calculateFinalLeans(digitMonth,digitCutoff), selectedActual=(file26?.records||[]).find(r=>r.month===digitMonth&&r.day===digitCutoff), checkedGames=selectedActual?GAME_NAMES.map((game,p)=>{const predicted=finalLeans[p].result,value=selectedActual.games[p],actual=classifyLean(value).replace(' Lean',''),matched=predicted===actual;return {game,predicted,actual,value,matched};}):[], resultCheck=selectedActual?{day:digitCutoff,matches:checkedGames.filter(item=>item.matched).length,games:checkedGames}:null;
      ...
      {!ready ? <section>
        ...
        <div className="grid gap-5 lg:grid-cols-[220px_minmax(0,1fr)]">
          ...
          <div className={`min-w-0 ${focus==='Single-digit patterns'||focus==='Odd/even patterns'?'pattern-special':''}`}>
            ...
            {focus==='Odd/even patterns'&&<OddEvenView month={digitMonth} cutoff={digitCutoff} setMonth={setDigitMonth} setCutoff={setDigitCutoff} rows={directLeanRows} finalLeans={finalLeans} resultCheck={resultCheck}/>}
